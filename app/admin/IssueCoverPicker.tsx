...
/** Number of fallback-video slots the admin can pin. Bumped from 3 → 10
 *  so the hero's video fallback can rotate further when there are no
 *  cover photos uploaded yet. Content keys are `works.{1..N}.videoId`,
 *  defined in `lib/content.ts`. */
const SLOT_COUNT = 10;
const SLOT_KEYS = Array.from(
  { length: SLOT_COUNT },
  (_, i) => `works.${i + 1}.videoId`,
);

/**
 * Admin picker for the home hero slideshow ("이번 호의 한 컷").
 *
 * Pins up to {@link SLOT_COUNT} videos that the home hero cycles through.
 * Selection is stored in the `works.{1..N}.videoId` content keys (same
 * keys the public site already reads). Saves through the existing
 * `/api/admin/content` PUT.
 *
 * UX:
 *   · Top: numbered slots showing the currently pinned thumbnails.
 *   · Bottom: full longform list. Click a row → fills the next empty slot.
 *     Click a filled slot → clears it.
 *   · Click 저장 → writes all slot keys in parallel + router.refresh().
 */
export default function IssueCoverPicker({
  ...
  // Pad the incoming initial array out to SLOT_COUNT entries (server
  // currently passes 3 — soon to be 10 — and we want to handle either
  // gracefully without crashing).
  const padded = (() => {
    const a = [...initial];
    while (a.length < SLOT_COUNT) a.push("");
    return a.slice(0, SLOT_COUNT);
  })();
  ...
  return (
    <div className="space-y-6">
      {/* ── Selected slots ─────────────────────────────
          With SLOT_COUNT bumped to 10, drop to a 5-col grid on lg+
          (and 2-col on sm/3-col on md) so two rows of pins fit
          without scrolling. */}
      <section>
      ...
      <div className="sticky top-[60px] z-10 flex flex-wrap items-center justify-between gap-3 px-4 py-3 rounded-xl bg-white border border-zinc-200 shadow-sm">
        <div className="text-sm text-zinc-600">
          <span className="font-bold text-zinc-900">
            {slots.filter(Boolean).length} / {SLOT_COUNT}
          </span>{" "}
          선택됨
          {error && (
